...
lemma "h1 ++ h2 ++ h3 = h1 ++ (h2 ++ h3)"
apply (simp only: hsimps(4))
done
...
(*apply (simp add: hsimps(4))
apply (intro exI conjI, (simp add: hsimps(4))+)+*)
...
by (smt disjoint_simps(3) disjoint_simps(4) dom_map_add list.set_intros(1) list.set_intros(2) map_add_assoc preciseD)
